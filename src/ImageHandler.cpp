...
void ImageHandler::Initialize(const int xSize, const int ySize, SDL_Renderer* pRenderer)
{
    ...
    this->TexturePtr = SDL_CreateTexture(this->RendererPtr, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING, this->XSize, this->YSize);
    this->InitColors();
}

// Function to set pixels.
void ImageHandler::SetPixel(const int x, const int y, const uint8_t red, const uint8_t green, const uint8_t blue)
{
...
void ImageHandler::Display()
{
    ...
    SDL_UpdateTexture(this->TexturePtr, NULL, this->PixelColor.data(), this->XSize * sizeof(uint32_t));
